...
def test_progress_text_contains_stage_and_elapsed():
    txt = progress_text("Scanning X", "fetching data", started=100.0, tick=2, now=190.0)
...
def test_monitor_view_renders_all_sections(conn):
    ...
    _seed_equity(conn)
    text = monitor_view(
        tick=3, pending_proposals=1, pending_exits=0, next_events=["Exit rule evaluation 15:30 ET"]
    )
