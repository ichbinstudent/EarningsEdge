...
def test_rich_msg_orders(seeded_db):
    ...
    for t in tracked:
        assert t.accessed_keys <= set(t.keys()), (
            f"view accessed unknown key(s): {t.accessed_keys - set(t.keys())}"
        )
...
def test_rich_msg_equity(seeded_db):
    ...
    with (
        unittest.mock.patch("earnings_edge.db.equity_snapshots_daily_avg", return_value=tracked),
        unittest.mock.patch(
            "framework.risk.equity.latest_equity",
            return_value={"equity": 10000.50, "buying_power": 5000.0, "ts": "2026-09-04"},
        ),
