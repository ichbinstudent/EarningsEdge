...
class TestBaseCollector(unittest.TestCase):
    ...
    def test_circuit_breaker_opens_after_failures(self):
        c = BaseCollector(name="test", max_retries=1, base_delay=0.01, circuit_threshold=2)
        for _ in range(2):
            try:
    ...
    def test_circuit_breaker_resets_on_success(self):
        c = BaseCollector(name="test", max_retries=3, base_delay=0.01, circuit_threshold=2)
...
class TestEarningsCalendarCollector(unittest.TestCase):
    def test_falls_back_to_finnhub_when_investing_fails(self):
        ...
        with (
            patch.object(collector, "_investing_fetch", return_value=[]),
            patch.object(
                collector,
                "_finnhub_fetch",
    ...
    def test_merges_sources_deduplicating(self):
        ...
        with (
            ...
            patch.object(
                ...
                "_finnhub_fetch",
                return_value=[
                    EarningsCandidate(ticker="AAPL", timing="Post Market", source="finnhub"),
...
class TestLSECollector(unittest.TestCase):
    ...
    def test_daily_bars_polygon_shape(self):
        ...
        client.candles.assert_called_once_with(
            "AAPL", "1d", start="2026-07-20", end="2026-07-21", limit=5000, order="asc"
        )

    def test_option_contracts_filters_expiry_window(self):
        client = MagicMock()
        client.options.return_value = [
            {"ticker": "A", "strike": 100.0, "expiry": "2026-08-01", "contract_type": "call"},
            {"ticker": "B", "strike": 105.0, "expiry": "2026-08-15", "contract_type": "put"},
            {"ticker": "C", "strike": 110.0, "expiry": "2026-12-31", "contract_type": "call"},
        ]
        c = self._make(client)
        out = c.option_contracts(
            "AAPL", as_of=date(2026, 7, 20), expiry_gte=date(2026, 7, 25), expiry_lte=date(2026, 9, 1)
        )
