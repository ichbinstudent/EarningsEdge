...
from scripts.ff_universe_scan import process_ticker


def test_pick_expiries_reused():
    import scripts.ff_universe_scan

    assert scripts.ff_universe_scan.pick_expiries.__module__ == "scripts.ff_backfill", (
        "pick_expiries must be imported from scripts.ff_backfill"
    )
...
def test_no_earnings_in_window(mock_iv, mock_hist, mock_earnings, mock_pick, mock_chain, mock_spot):
    ...
    mock_pick.return_value = (
        {
            "expiry": "2026-09-15",
            "dte": 30,
            "contracts": [{"ticker": "C1", "strike_price": 100, "close": 5.0, "contract_type": "call"}],
        },
        {
            "expiry": "2026-10-15",
            "dte": 60,
            "contracts": [{"ticker": "C2", "strike_price": 100, "close": 6.0, "contract_type": "call"}],
        },
    )
...
@patch("scripts.ff_universe_scan.hist_move_stats")
@patch("scripts.ff_universe_scan.implied_volatility")
def test_with_earnings_in_window(mock_iv, mock_hist, mock_earnings, mock_pick, mock_chain, mock_spot):
    mock_spot.return_value = 100.0
    mock_chain.return_value = [
        {
            "expiration_date": "2026-10-01",
