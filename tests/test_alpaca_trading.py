...
def mock_client():
    ...
    client.position_symbols = MagicMock(return_value=set())
...
def test_skip_existing_position(mock_client):
    ...
    mock_client.position_symbols = MagicMock(return_value={"AAPL250117C00150000"})
    bridge = StrategyBridge(client=mock_client, config=BridgeConfig(dry_run=True))
    trade = _make_trade("DIRECTIONAL_CALL")
    result = bridge.execute_trade(trade)
    assert result is None  # skipped
    assert bridge.skip_reasons["position_exists"] == 1
...
def test_occ_symbol_makes_zero_api_calls(mock_client):
    """OCC construction is pure string building — no contract-catalog call."""
    bridge = StrategyBridge(client=mock_client, config=BridgeConfig(dry_run=False))
    sym = bridge._occ_symbol("AAPL", date(2025, 1, 17), 150.0, "call")
    assert sym == "AAPL250117C00150000"
    mock_client.get_option_contracts.assert_not_called()


def test_position_set_fetched_once_per_run(mock_client):
    """Position symbols come from ONE batched call, cached for the whole run —
    not one API call per leg per candidate."""
    mock_client.position_symbols = MagicMock(return_value=set())
    bridge = StrategyBridge(client=mock_client, config=BridgeConfig(dry_run=True))
    for _ in range(3):
        bridge.execute_trade(_make_trade("DIRECTIONAL_CALL"))
    mock_client.position_symbols.assert_called_once()


def test_position_symbols_batch(mock_client):
    del mock_client.position_symbols  # un-stub: exercise the real method
    mock_client.session.request.return_value = MagicMock(
        status_code=200,
        json=lambda: [{"symbol": "AAPL250117C00150000"}, {"symbol": "TSLA250117P00200000"}],
    )
    assert mock_client.position_symbols() == {"AAPL250117C00150000", "TSLA250117P00200000"}
...
def test_submit_retries_once_with_catalog_resolution(mock_client):
    ...
    assert mock_client.submit_multi_leg_order.call_count == 2
    # catalog hit once per distinct expiry window (near + far)
    assert mock_client.get_option_contracts.call_count == 2


def test_submit_no_retry_on_non_symbol_error(mock_client):
    """A 5xx/permission rejection must NOT trigger the resolution retry."""
    mock_client.submit_order = MagicMock(side_effect=AlpacaError(403, "forbidden"))
    mock_client.position_symbols = MagicMock(return_value=set())
    bridge = StrategyBridge(client=mock_client, config=BridgeConfig(dry_run=False))
    result = bridge.execute_trade(_make_trade("DIRECTIONAL_CALL"))
    assert result is None
    assert mock_client.submit_order.call_count == 1
    mock_client.get_option_contracts.assert_not_called()
    assert bridge.skip_reasons["alpaca_error"] == 1
...
def test_get_and_cancel_order_url_paths(mock_client):
    ...
    mock_client.session.request.return_value = MagicMock(status_code=200, json=lambda: {"id": "o1"})
    mock_client.get_order("o1")
    url = mock_client.session.request.call_args[0][1]
    assert url.endswith("/orders/o1")
    mock_client.cancel_order("o1")
    url = mock_client.session.request.call_args[0][1]
    assert url.endswith("/orders/o1")


def test_no_retry_on_404(mock_client):
    mock_client.session.request.return_value = MagicMock(status_code=404, json=lambda: {"message": "nf"})
    with pytest.raises(AlpacaNotFoundError):
        mock_client.get_order("missing")
    assert mock_client.session.request.call_count == 1


def test_no_retry_on_401(mock_client):
    mock_client.session.request.return_value = MagicMock(status_code=401, json=lambda: {"message": "no"})
    with pytest.raises(AlpacaAuthError):
        mock_client.get_order("x")
    assert mock_client.session.request.call_count == 1
