...
async def cmd_signals(bot, update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await bot._send_panel(
        update, bot._signals_intro(), reply_markup=bot._signals_kb(uid, modes), parse_mode=HTML
    )
...
async def cmd_setups(bot, update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await bot._send_panel(update, setup_menu_text(), reply_markup=InlineKeyboardMarkup(ikb), parse_mode=HTML)
...
async def _cmd_set_lifecycle(bot, update: Update, promote: bool) -> None:
    ...
    if len(args) != 2:
        ...
        return
    cur, new = await asyncio.to_thread(bot._lifecycle_step_sync, args[1], promote, "telegram")
...
async def cmd_settings(bot, update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    await bot._send_panel(update, "🛠 <b>Settings</b>", parse_mode=HTML, reply_markup=bot._settings_kb())
...
async def cmd_orders(bot, update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    success = await send_rich_html(
        bot.application.bot, update.effective_chat.id, html, reply_markup=desk_refresh_kb("or")
    )
    if not success:
...
async def cmd_jobs(bot, update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    success = await send_rich_html(
        bot.application.bot, update.effective_chat.id, html, reply_markup=desk_refresh_kb("jb")
    )
    if not success:
...
async def cmd_equity(bot, update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    success = await send_rich_html(
        bot.application.bot, update.effective_chat.id, html, reply_markup=desk_refresh_kb("eq")
    )
    if not success:
...
async def cmd_exits(bot, update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(cards.batch_overview(counts, "exit"), parse_mode=HTML)
...
async def cmd_propose(bot, update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    pm = ProgressMessage(bot.application.bot, update.effective_chat.id, "Building trade proposals")
    await pm.start()
    try:
