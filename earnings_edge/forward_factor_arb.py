...
def build_candidate(alpaca, ticker: str, *, today: date | None = None):
    ...
    mid_debit = combo_debit(
        q1.get("bid", 0), q1.get("ask", 0), q2.get("bid", 0), q2.get("ask", 0), executable=False
    )
    if not mid_debit:
    ...
    return CalendarCandidate(
        ...
        near_bid=near_bid,
        near_ask=q1.get("ask", 0),
        far_bid=q2.get("bid", 0),
        far_ask=far_ask,
        sigma_fwd=fwd_vol,
        hist_rms_move=rms or 0.0,
        tau_days=0,
        d_cap=round(debit_cap, 2),
