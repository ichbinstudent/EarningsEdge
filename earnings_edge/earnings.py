...
def _investing_earnings(date_: date) -> List[EarningsCandidate]:
    ...
    try:
        if _HAS_CFFI:
            resp = cffi_requests.post(url, headers=headers, data=payload, timeout=10, impersonate="chrome")
        else:
...
def scan_dates(
    ...
    Weekend-aware: rolls forward to the next trading day (Mon-Fri).
    """
    if input_date:
        try:
        ...
        return _next_trading_day(post), _next_trading_day(post + timedelta(days=1))
    ...
    post = _next_trading_day(post)
    pre = _next_trading_day(post + timedelta(days=1))
    return post, pre
