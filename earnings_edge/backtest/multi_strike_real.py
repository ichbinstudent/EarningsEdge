...
def strike_for_delta(underlying: float, T: float, r: float, sigma: float, target_delta: float) -> float:
    ...
    K = (
        underlying * (1 + math.sqrt(T) * sigma * 1.04)
        if target_delta > 0
        else underlying * (1 - math.sqrt(T) * sigma * 1.04)
    )
    for _ in range(50):
...
def pull_bid_ask(
    chain_df: pd.DataFrame, contract_ticker: str
...
class IronCondorReal:
    ...
    def run(self, bundle: DataBundle) -> StrategyResult:
        ...
        for _, row in filtered.iterrows():
            ...
            sc_call_bid, sc_call_ask = api.bid_ask_or_bs(
                chain,
                f"{ticker}{expiry.strftime('%y%m%d')}C{int(sc_call_k * 1000):08d}",
                S,
                sc_call_k,
                T,
                iv,
            ...
            sp_put_bid, sp_put_ask = api.bid_ask_or_bs(
                chain,
                f"{ticker}{expiry.strftime('%y%m%d')}P{int(sp_put_k * 1000):08d}",
                S,
                sp_put_k,
                T,
                iv,
            ...
            lc_call_bid, lc_call_ask = api.bid_ask_or_bs(
                chain,
                f"{ticker}{expiry.strftime('%y%m%d')}C{int(lc_call_k * 1000):08d}",
                S,
                lc_call_k,
                T,
                iv,
            ...
            lp_put_bid, lp_put_ask = api.bid_ask_or_bs(
                chain,
                f"{ticker}{expiry.strftime('%y%m%d')}P{int(lp_put_k * 1000):08d}",
                S,
                lp_put_k,
                T,
                iv,
            ...
            # PnL
            call_loss = max(0.0, underlying_at_expiry - sc_call_k) - max(
                0.0, underlying_at_expiry - lc_call_k
            )
            ...
            trades.append(
                Trade(
                    ...
                    pnl_pct=pnl_percent,
                    features={
                        "iv_rv": row.get("iv30_rv30"),
                        "expected_move_pct": row.get("expected_move_pct"),
                        "risk_reward": risk_reward,
                        "short_call": sc_call_k,
                        "short_put": sp_put_k,
                        "long_call": lc_call_k,
                        "long_put": lp_put_k,
                        "dte": dte,
                        "leg_premiums": [sc_call_bid, sp_put_bid, lc_call_ask, lp_put_ask],
                        # defined-risk margin per 1-lot: wing width minus credit
                        "margin_dollars": (max_loss - credit) * 100.0,
                    },
                    ml_decision="TAKE",
...
class ButterflyReal:
    ...
    def __init__(self, iv_rv_min: float = 1.15, min_expected_move: float = 6.0, width: int = 5):
        self.iv_rv_min = iv_rv_min
        self.min_expected_move = min_expected_move
        self.width = width
    ...
    def run(self, bundle: DataBundle) -> StrategyResult:
        ...
        for _, row in filtered.iterrows():
            ...
            hi = atm + self.width
            if lo <= 0:
                continue  # non-positive strike is not tradeable

            lo_bid, lo_ask = api.bid_ask_or_bs(
                chain, f"{ticker}{expiry.strftime('%y%m%d')}C{int(lo * 1000):08d}", S, lo, T, iv, "call"
            )
            atm_bid, atm_ask = api.bid_ask_or_bs(
                chain, f"{ticker}{expiry.strftime('%y%m%d')}C{int(atm * 1000):08d}", S, atm, T, iv, "call"
            )
            hi_bid, hi_ask = api.bid_ask_or_bs(
                chain, f"{ticker}{expiry.strftime('%y%m%d')}C{int(hi * 1000):08d}", S, hi, T, iv, "call"
            )
            ...
            trades.append(
                Trade(
                    ...
                    features={
                        ...
                        # 4 contract fills: 1x lo, 2x ATM body, 1x hi
                        "leg_premiums": [lo_ask, atm_bid, atm_bid, hi_ask],
                        # long fly max loss = debit paid
                        "margin_dollars": net_premium * 100.0,
                    },
                    ml_decision="TAKE",
...
class RiskReversalReal:
    ...
    def run(self, bundle: DataBundle) -> StrategyResult:
        ...
        for _, row in filtered.iterrows():
            ...
            call_bid, call_ask = api.bid_ask_or_bs(
                chain, f"{ticker}{expiry.strftime('%y%m%d')}C{int(kc * 1000):08d}", S, kc, T, iv, "call"
            )
            put_bid, put_ask = api.bid_ask_or_bs(
                chain, f"{ticker}{expiry.strftime('%y%m%d')}P{int(kp * 1000):08d}", S, kp, T, iv, "put"
            )

            # Buy OTM call (pay ask), sell OTM put (receive bid)
            net_premium = call_ask - put_bid

            # Undefined risk (naked short put): REG-T margin per 1-lot
            margin_dollars = regt_margin(
                [
                    OptionLeg("buy", "call", kc, S, call_ask),
                    OptionLeg("sell", "put", kp, S, put_bid),
                ]
            )
