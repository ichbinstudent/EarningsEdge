...
if __name__ == "__main__":
    with get_session() as session:
        ...
        def q(p):
            return pr[min(n - 1, int(p * n))]

        print(f"\n=== PREMIUM RATIO (n={n}) ===")
        print(
            f"  min {pr[0]:.2f} | p25 {q(0.25):.2f} | median {q(0.5):.2f} | p75 {q(0.75):.2f} | p90 {q(0.9):.2f} | max {pr[-1]:.2f}"
        )
        ...
        for r in rows:
            out = session.execute(
                ...
                {"ticker": r["ticker"], "scan_date": r["scan_date"]},
            ).fetchone()
        ...
        for b, rs in buckets.items():
            ...
            print(
                f"  {b:8s}: n={m:3d} | overpriced (actual<implied) {win / m:.0%} | "
