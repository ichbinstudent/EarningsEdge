...
class TradingBot:
    ...
    async def _exit_eval(self):
        ...
        pushed = getattr(self, "_exit_cards_pushed", None)
        if pushed is None:
            pushed = self._exit_cards_pushed = set()
        new_rows = [r for r in stats.get("pending_cards", []) if r["id"] not in pushed]
        for row in new_rows:
            pushed.add(row["id"])
        if new_rows:
            for uid in self._approval_chats():
                await self._push_batches(uid, new_rows, "exit")
    ...
    async def _picks_pipeline(self):
        ...
        for uid in self._approval_chats():
            try:
                ...
                logger.error("picks push to %d failed: %s", uid, exc)
