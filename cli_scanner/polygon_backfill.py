...
def snapshot_exists(ticker: str, earnings_date: date) -> bool:
    return snapshots_exists(ticker, earnings_date.isoformat())
...
def run_backfill(
    ...
    for i, event in enumerate(events, start=1):
        ...
        try:
            if not dry_run and snapshot_exists(event.ticker, event.earnings_date):
