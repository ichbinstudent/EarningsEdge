...
# ─── HELPERS ────────────────────────────────────────────────────────────
from earnings_edge.db.engine import configure as _db_configure
...
def load_calendar_trades() -> pd.DataFrame:
    df = calendar_call_trades_with_features_df()
    if df.empty:
        return df
...
# ─── HEADER BAR ─────────────────────────────────────────────────────────
_counts = snapshots_status_counts()
total = _counts["total"]
labeled = _counts["labeled"]
pending = _counts["pending"]
trades = calendar_call_trades_count()
...
with tab_health:
    ...
    df_out = snapshots_outcome_moves_df()
...
with tab_live:
    ...
    try:
        df_live = live_calendar_candidates_recent_df(100)
    ...
    try:
        df_out = scanner_scan_outputs_recent_df(100)
        if not df_out.empty:
...
with tab_queue:
    ...
    _counts = snapshots_status_counts()
    pending = _counts["pending"]
    unavailable = _counts["unavailable"]
    labeled = _counts["labeled"]
    total = _counts["total"]
    ...
    df_queue = snapshots_queue_df(queue_view, ticker_filter=ticker_filter, limit=int(limit))
        ...
        if queue_view == "Unavailable" and len(df_queue) > 0:
            ...
            if reset_ids and not reset_all:
                if st.button(f"Reset {len(reset_ids)} to Pending", type="primary"):
                    snapshots_reset_outcomes(list(reset_ids))
                    st.success(f"Reset {len(reset_ids)} snapshots to Pending. Refresh to see changes.")
                    st.rerun()

            elif reset_all:
                snapshots_reset_outcomes_view(queue_view, ticker_filter=ticker_filter)
    ...
    _bf = calendar_call_trades_backfill_stats()
    trades_total = _bf["total"]
    trades_with_exit = _bf["with_exit"]
    trades_no_exit = _bf["no_exit"]
    date_range = (_bf["min_scan_date"], _bf["max_scan_date"])
