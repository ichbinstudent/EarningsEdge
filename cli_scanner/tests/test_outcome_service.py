...
class TestOutcomeFromBars(unittest.TestCase):
    ...
    def test_amc_uses_close_after_earnings_date(self):
        """AMC / Post Market: pre = ed close, post = first close after ed."""
        ed = date(2026, 7, 29)
        bars = [
            _bar(ed - timedelta(days=1), c=100.0, h=101.0, l=99.0),
        ...
        # BMO/default uses pre=100 → ed=99.3
        self.assertAlmostEqual(bmo["pre_earnings_close"], 100.0, places=2)
        self.assertAlmostEqual(bmo["post_earnings_close"], 99.3, places=2)
        amc = OutcomeService.outcome_from_bars(bars, ed, timing="Post Market")
        self.assertAlmostEqual(amc["pre_earnings_close"], 99.3, places=2)
        self.assertAlmostEqual(amc["post_earnings_close"], 84.5, places=2)
        # Must NOT be the flattering pre→ed move.
        self.assertLess(amc["actual_move_pct"], -10.0)
...
class TestRunOutcomes(unittest.TestCase):
    ...
    def test_limit_caps_processing(self):
        ...
        bars = [
            _bar(ed - timedelta(days=1), c=90.0, h=91.0, l=89.0),
            _bar(ed, c=100.0, h=101.0, l=99.0),
            _bar(ed + timedelta(days=1), c=110.0, h=112.0, l=108.0),
        ]
        service = OutcomeService(polygon_client=FakePolygon(bars), db_path=self.db_path)
    ...
    def test_pending_outcomes_prefer_has_options(self):
        ...
        self.assertLess(tickers.index("LIQUID"), tickers.index("OTCJUNK"))
