...
class StockValidator:
    ...
    def __init__(self, analyzer: OptionsAnalyzer, browser: MarketChameleonBrowser, provider=None) -> None:
    ...
    def validate(self, candidate: EarningsCandidate) -> ValidationResult:
        ...
        try:
            ...
            options_dates = provider.options_expiries(ticker)
            ...
            # 4. Open interest (skipped when the backend has no OI data,
            #    e.g. the Polygon fallback — same pattern as the win-rate gate)
            chain = provider.option_chain(ticker, options_dates[0])
            if chain.oi_available:
            ...
            if raw != "N/A":
                em_fail = self._check_expected_move(raw, price, m, provider, options_dates, ticker)
                if em_fail:
                    return em_fail

            # 9. Volume
            vol = provider.history(ticker, "1mo")["Volume"].mean()
            m.volume = vol
            if vol < 1_000_000:
