...
class Settings:
    ...
    # Scanner schedule (cron: Berlin TZ) — 14:00 ET
    scanner_schedule: str = "0 20 * * mon-fri"
...
def reset_settings() -> None:
    """Drop the singleton so the next get_settings() re-reads os.environ.

    Settings is process-wide and computed once on first access; tests that
    patch env vars (e.g. LSE_API_KEY) around a call must reset this both
    before and after, or whichever test happens to trigger the first
    get_settings() call bakes its patched env into every later test.
    """
    global _settings
    _settings = None
