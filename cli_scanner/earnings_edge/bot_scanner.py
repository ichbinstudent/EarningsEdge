...
# Live calendar quotes with debit > this fraction of spot are garbage
# (crossed combo, wrong root, or a 10-delta masquerading as ATM).
MAX_DEBIT_PCT_OF_SPOT = 0.15
...
def quote_is_sane(
    ...
    net_debit: float,
    spot: float,
    max_debit_pct: float = MAX_DEBIT_PCT_OF_SPOT,
) -> bool:
    """Reject inverted bid/ask or a debit that is implausibly large vs spot."""
    if near_ask < near_bid or far_ask < far_bid:
        return False
    if spot and spot > 0 and net_debit > 0 and (net_debit / spot) > max_debit_pct:
        return False
    return True
...
def select_live_calendar_call_quote(
    ...
    bid/ask mids as a display/ranking estimate rather than an executable
    quote; on the Polygon fallback bid/ask collapse to the EOD close, which
    matches the backtest's pricing convention.
    """
    ...
    try:
        ...
        expiries = [
            exp
            for exp in sorted(provider.options_expiries(ticker))
        ...
        near_calls = provider.option_chain(ticker, near_expiry).calls
        far_calls = provider.option_chain(ticker, far_expiry).calls
...
class EarningsCalendarScanner(BaseScanner):
    ...
    Runs daily at 14:00 ET (20:00 Berlin) on weekdays — early enough that
    the scan (30–60 min on heavy days) finishes and the chained trade
    proposals land well before the 16:00 ET close.
    """
    ...
    def scan(self) -> Dict[str, Any]:
        ...
        svc = ScanService(scanner=self, db_path=getattr(self, "_db_path", None))
    ...
    def _persist_live_calendar_candidate(
        ...
        *,
        ...
        try:
            ...
            insert_live_calendar_candidate(row)
        except Exception:
