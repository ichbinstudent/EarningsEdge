...
def load_calendar_trades(db_path: Path) -> pd.DataFrame:
    ...
    """
    df = pd.read_sql(text(query), get_engine(), parse_dates=["earnings_date", "scan_date"])
    if df.empty:
        return df
...
def cv_evaluate(
    ...
    *,
    ...
    """Time-series cross-validation over the gated dataframe (no artifact persistence).

    ``clean`` is the output of apply_data_quality_gates (already date-sorted;
    re-sorted by earnings_date defensively). A fresh build_model_pipeline() is
    fit per TimeSeriesSplit fold. Classification folds whose test segment holds
    a single class record NaN for all metrics (roc_auc_score would raise).
    """
    data = clean.sort_values("earnings_date").reset_index(drop=True)
    y = make_target(data, target, min_pnl, min_return)
    regression = is_regression_target(target)
    if not regression:
        y = y.astype(int)
    X = data[features].apply(pd.to_numeric, errors="coerce")

    splits = min(n_splits, 5)
...
def main() -> None:
    ...
    parser.add_argument(
        "--cv",
        action="store_true",
        help="Run time-series cross-validation only; prints JSON, writes no artifacts",
    )
    ...
    if args.cv:
        # Same load/gates/filter/feature-discovery path as train(), then pure
        # evaluation: no artifact or metadata file is written.
        df = load_calendar_trades(args.db)
        clean, _quality = apply_data_quality_gates(df, args.max_moneyness_error)
        if args.min_iv_rv is not None:
            clean = clean[clean["iv30_rv30"] >= args.min_iv_rv].copy()
        if len(clean) < args.min_rows:
            raise RuntimeError(f"Not enough clean rows: {len(clean)} < {args.min_rows}")
